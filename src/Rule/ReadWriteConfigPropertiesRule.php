 ...
 */
class ReadWriteConfigPropertiesRule extends UnusedPrivatePropertyRule
{
        ...
        if ($classReflection->hasTraitUse(Configurable::class) || $classReflection->isSubclassOf(Extension::class)) {
            ...
            foreach ($errors as $error) {
                ...
                if ($nameFound) {
                    ...
                    $hints[] = RuleErrorBuilder::message(sprintf('Have you forgotten to add "@config" for the property %s of the configurabel class %s?', $names[2],$names[1]))->line($error->line)->tip($tip)->build();
                }
            }
            return $hints;
        }
